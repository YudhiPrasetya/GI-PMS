<style>
    .sweet_loader {
        width: 140px;
        height: 140px;
        margin: 0 auto;
        animation-duration: 0.5s;
        animation-timing-function: linear;
        animation-iteration-count: infinite;
        animation-name: ro;
        transform-origin: 50% 50%;
        transform: rotate(0) translate(0, 0);
    }
</style>
<div class="page-wrapper">
    <div class="page-content">

        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Master</div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        <li class="breadcrumb-item"><a href="javascript:;"><i class="bx bx-home-alt"></i></a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">Master User</li>
                    </ol>
                </nav>
            </div>
        </div>
        <!--end breadcrumb-->
        <h6 class="mb-0 text-uppercase">Master User</h6>
        <hr />

        <div class="card">
            <div class="card-body">
                <div class="row my-3 mx-2">
                    <div class="col mb-3">


                        <div class="accordion" id="accordionExample">
                            <div class="accordion-item" style="border: 0px;">
                                <h2 class="accordion-header" id="headingOne">
                                    <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne" id="btn_create_user">
                                        <i class='bx bx-plus-circle'></i> Create User
                                    </button>
                                </h2>
                                <div id="collapseOne" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                                    <div class="accordion-body">
                                        <div class="row">

                                            <div class="col-lg-12">
                                                <div class="row g-3 mb-3">
                                                    <div class="col-lg-2">
                                                        <label for="username" class="col-form-label">User Name</label>
                                                    </div>
                                                    <div class="col-lg-8">
                                                        <textarea class="form-control" id="username" style="height: 30px"></textarea>
                                                    </div>
                                                </div>
                                                <div class="row g-3 mb-3">
                                                    <div class="col-lg-2">
                                                        <label for="password" class="col-form-label">Password</label>
                                                    </div>
                                                    <div class="col-lg-7">
                                                        <textarea class="form-control" id="password" style="height: 30px"></textarea>
                                                        <div id="warningPassword" class="form-text" style="color:crimson;font-size:80%">
                                                            (Password and Confirm Password are not the same!)
                                                        </div>
                                                    </div>
                                                </div>
                                                <div id="confirmPasswordAll" class="row g-3 mb-3">
                                                    <div class="col-lg-2">
                                                        <label for="confirmPassword" class="col-form-label">Confirm Password</label>
                                                    </div>
                                                    <div class="col-lg-7">
                                                        <input class="form-control" id="confirmpassword" placeholder="Confirm Password"></input>
                                                        <div id="warningConfirm" class="form-text" style="color:crimson;font-size:80%">
                                                            (Password and Confirm Password are not the same!)
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row g-3 mb-3">
                                                    <div class="col-lg-2">
                                                        <label for="role" class="col-form-label">Role</label>
                                                    </div>
                                                    <div class="col-lg-3">
                                                        <select id="role" class="form-control select2_1" name="role">
                                                        </select>
                                                    </div>
                                                    <div class="col-lg-1">
                                                        <label for="role" class="col-form-label">Factory</label>
                                                    </div>
                                                    <div class="col-lg-3">
                                                        <select id="factory" class="form-control select2_1" name="factory">
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="row g-3 mb-3">
                                                    <div class="col-lg-2">
                                                    </div>
                                                    <div class="col-lg-8">
                                                        <button class="btn btn-primary" id="btn_save">Save</button>
                                                        <button class="btn btn-outline-secondary" id="btn_reset">Reset</button>
                                                    </div>
                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>


                    </div>
                    <!-- <div class="table-responsive"> -->
                    <table id="masterUserTable" class="table table-striped table-bordered table-sm nowrap" style="width:100%">
                        <thead>
                            <tr>
                                <th>No.</th>
                                <th>User Name</th>
                                <th>Role</th>
                                <th>Factory</th>
                                <th>Active</th>
                                <th>Action</th>
                                <!-- <th class="text-center">Quote</th>
                                <th>Action</th> -->
                            </tr>
                        </thead>
                    </table>
                    <!-- </div> -->
                </div>
            </div>
        </div>
    </div>
</div>
<!-- Modal -->
<!-- Modal Edit Order -->
<div class="modal fade" id="editUserModal" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit User</h5>
                <button id="closeModal" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="bodyUserModal">
                <div class="row mx-3">

                    <div class="col-lg-12">
                        <div class="row g-3 mb-3">
                            <div class="col-lg-2">
                                <label for="username" class="col-form-label">User Name</label>
                            </div>
                            <div class="col-lg-8">
                                <textarea class="form-control" id="usernameModal" style="height: 30px"></textarea>
                            </div>
                        </div>
                        <div class="row g-3 mb-3">
                            <div class="col-lg-2">
                                <label for="password" class="col-form-label">New Password</label>
                            </div>
                            <div class="col-lg-7">
                                <input class="form-control" id="passwordModal" placeholder="Enter New Password"></input>
                                <div id="infoNewPasswordModal" class="form-text" style="color:crimson;font-size:80%">
                                    (If you do not want to change your password, leave this field empty)
                                </div>
                                <div id="warningNewPasswordModal" class="form-text" style="color:crimson;font-size:80%">
                                    (New Password and Confirm Password are not the same!)
                                </div>
                            </div>
                        </div>
                        <div id="confirmNewPasswordModal" class="row g-3 mb-3">
                            <div class="col-lg-2">
                                <label for="password" class="col-form-label">Confirm Password</label>
                            </div>
                            <div class="col-lg-7">
                                <input class="form-control" id="confirmpasswordModal" placeholder="Confirm Password"></input>
                                <div id="warningConfirmNewPasswordModal" class="form-text" style="color:crimson;font-size:80%">
                                    (New Password and Confirm Password are not the same!)
                                </div>
                            </div>
                        </div>
                        <div class="row g-3 mb-3">
                            <div class="col-lg-2">
                                <label for="role" class="col-form-label">Role</label>
                            </div>
                            <div class="col-lg-3">
                                <select id="roleModal" class="form-control select2_2" name="role">
                                </select>
                            </div>
                            <div class="col-lg-1">
                                <label for="role" class="col-form-label">Factory</label>
                            </div>
                            <div class="col-lg-3">
                                <select id="factoryModal" class="form-control select2_2" name="factory">
                                </select>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <!-- <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button> -->
                    <button type="button" id="btn_save_edit_user" class="btn btn-primary">Save changes</button>
                </div>
            </div>
        </div>
    </div>
</div>
<script>
    $('.select2_1').select2({
        theme: 'bootstrap-5',
        width: $(this).data('width') ? $(this).data('width') : $(this).hasClass('w-100') ? '100%' : 'style',
        // placeholder: $(this).data('placeholder'),
        // allowClear: Boolean($(this).data('allow-clear')),
        // dropdownParent: $('#createNewOrderModal')
    });
    $('.select2_2').select2({
        theme: 'bootstrap-5',
        width: $(this).data('width') ? $(this).data('width') : $(this).hasClass('w-100') ? '100%' : 'style',
        dropdownParent: $('#bodyUserModal')
    });
</script>
<script>
    const sweet_loader = '<div class="sweet_loader"><svg viewBox="0 0 140 140" width="140" height="140"><g class="outline"><path d="m 70 28 a 1 1 0 0 0 0 84 a 1 1 0 0 0 0 -84" stroke="rgba(0,0,0,0.1)" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"></path></g><g class="circle"><path d="m 70 28 a 1 1 0 0 0 0 84 a 1 1 0 0 0 0 -84" stroke="#71BBFF" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-dashoffset="200" stroke-dasharray="300"></path></g></svg></div>';

    $(document).ready(function() {
        $('#confirmNewPasswordModal').hide();
        $('#infoNewPasswordModal').show();
        $('#warningNewPasswordModal').hide();
        $('#warningNewConfirmPasswordModal').hide();
        $('#warningConfirmNewPasswordModal').hide();
        $('#warningPassword').hide();
        $('#warningConfirm').hide();

        $('#factory').empty();
        $.ajax({
            url: "<?= site_url('admin/getMasterFactory') ?>",
            type: 'GET',
            dataType: 'JSON',
            // data: {},
            success: function(factory) {
                $('#factory').append($('<option>', {
                    value: "",
                    text: "-- Select Factory --"
                }));
                $.each(factory, function(index, item) {
                    $('#factory').append($('<option>', {
                        value: item.idFactory,
                        text: item.Factory
                    }));

                });
            }
        })
        $('#role').empty();
        $.ajax({
            url: "<?= site_url('admin/getMasterRole') ?>",
            type: 'GET',
            dataType: 'JSON',
            // data: {},
            success: function(role) {
                $('#role').append($('<option>', {
                    value: "",
                    text: "-- Select role --"
                }));
                $.each(role, function(index, item) {
                    $('#role').append($('<option>', {
                        value: item.id_role,
                        text: item.name_role
                    }));

                });
            }
        })
        $('#factoryModal').empty();
        $.ajax({
            url: "<?= site_url('admin/getMasterFactory') ?>",
            type: 'GET',
            dataType: 'JSON',
            // data: {},
            success: function(factory) {
                $('#factoryModal').append($('<option>', {
                    value: "",
                    text: "-- Select Factory --"
                }));
                $.each(factory, function(index, item) {
                    $('#factoryModal').append($('<option>', {
                        value: item.idFactory,
                        text: item.Factory
                    }));

                });
            }
        })
        $('#roleModal').empty();
        $.ajax({
            url: "<?= site_url('admin/getMasterRole') ?>",
            type: 'GET',
            dataType: 'JSON',
            // data: {},
            success: function(role) {
                $('#roleModal').append($('<option>', {
                    value: "",
                    text: "-- Select role --"
                }));
                $.each(role, function(index, item) {
                    $('#roleModal').append($('<option>', {
                        value: item.id_role,
                        text: item.name_role
                    }));

                });
            }
        })
        let masterUserTable = $('#masterUserTable').DataTable({
            // lengthChange: false,
            // buttons: ['copy', 'excel', 'pdf', 'print'],
            scrollX: true,
            ajax: {
                url: '<?= site_url("admin/getMasterUser"); ?>',
                type: 'GET',
                dataType: 'JSON'
            },
            columns: [{
                    "data": null,
                    "orderable": true,
                    "searchable": false,
                    'className': 'text-center px-4',
                    // "width": "100px",
                    "render": function(data, type, row, meta) {
                        return meta.row + meta.settings._iDisplayStart + 1;
                    }
                },
                {
                    'data': 'user_name',
                    'className': 'text-center px-3'
                },
                {
                    'data': 'name_role',
                    'className': 'text-center px-3'
                },
                {
                    'data': 'Factory',
                    'className': 'text-center px-3'
                },
                {
                    'className': 'text-center px-3',
                    "width": "50px",
                    render: function(data, type, row) {
                        if (row['active'] == 0) {
                            return "<div class='form-check form-switch' style='padding-left: 45px'><input class='form-check-input' id='btn_active' type='checkbox' role='switch' checked></div>"
                        } else {
                            return "<div class='form-check form-switch' style='padding-left: 45px'><input class='form-check-input' id='btn_active' type='checkbox' role='switch'></div>"
                        }
                    }
                },
                {
                    'className': 'text-center px-3',
                    "width": "90px",
                    render: function(data, type, row) {
                        return '<span class="badge text-bg-info text-white" style="cursor: pointer;" id="btn_edit">Edit</span> <span class="badge text-bg-danger" style="cursor: pointer;" id="btn_delete">Delete</span>'
                    }
                },
                // {
                //     'className': 'text-center px-3',
                //     render: function(data, type, row) {
                //         if (row['active'] == 1) {
                //             return "<div class='form-check form-switch' style='padding-left: 45px'><input class='form-check-input' id='btn_active' type='checkbox' role='switch' checked></div>"
                //         } else {
                //             return "<div class='form-check form-switch' style='padding-left: 45px'><input class='form-check-input' id='btn_active' type='checkbox' role='switch'></div>"
                //         }
                //     }
                // },
                // {
                //     'className': 'text-center px-3',
                //     render: function(data, type, row) {
                //         return '<span class="badge text-bg-info text-white" style="cursor: pointer;" id="btn_edit">Edit</span> <span class="badge text-bg-danger" style="cursor: pointer;" id="btn_delete">Delete</span>'
                //     }
                // },

            ],

        });
        $('#btn_create_user').click(function() {
            $('#username').focus();
        })
        $('#btn_reset').click(function() {
            $('#username').val("");
            // $('#username').focus();
            $('#password').val("");
            $('#role').val("").change();
            $('#factory').val("").change();

            setTimeout(function() {
                $('#username').focus();
            }, 500)

        })
        $('#masterUserTable tbody').on('click', '#btn_active', function(e) {
            let id_user = masterUserTable.row($(this).parents('tr')).data().id_user;
            let isChecked = $(e.target).is(':checked');
            let active = (isChecked == true) ? 0 : 1;


            $.ajax({
                url: "<?= site_url('admin/updateActiveUser'); ?>",
                type: 'POST',
                data: {
                    "id_user": id_user,
                    "active": active,
                },
                success: function(data) {
                    if (active == 0) {
                        swal.fire(
                            'Success!',
                            'User is Aktif.',
                            'success'
                        ).then(function() {
                            // masterQuoteTable.ajax.reload();
                        })
                    } else if (active == 1) {
                        swal.fire(
                            'Success!',
                            'User is not Aktif.',
                            'success'
                        ).then(function() {
                            // masterQuoteTable.ajax.reload();
                        })
                    }

                }
            });


        });
        $('#btn_save').click(function() {
            let username = $('#username').val();
            let password = $('#password').val();
            let confirmpassword = $('#confirmpassword').val();
            let role = $('#role').val();
            let factory = $('#factory').val();

            $('#btn_save').prop('disabled', true);
            $('#btn_save').html('Saving..');

            if (password === confirmpassword) {
                if (username !== '' && password !== '' && role !== '' && factory !== '') {

                    $.ajax({
                        type: "POST",
                        url: "<?= site_url('admin/postUser') ?>",
                        dataType: "JSON",
                        data: {
                            'username': username,
                            'password': password,
                            'role': role,
                            'factory': factory,
                        },
                        success: function(data) {
                            masterUserTable.ajax.reload(null, false);
                            $('#warningPassword').hide();
                            $('#warningConfirm').hide();
                            swal.fire({
                                icon: 'success',
                                title: 'Success!',
                                text: 'Data saved successfully.',
                                timer: 1000,
                                showCancelButton: false,
                                showConfirmButton: false
                            }).then(function() {
                                $('#btn_save').prop('disabled', false);
                                $('#btn_save').html('Save');
                                $('#username').val("");
                                // $('#username').focus();
                                $('#password').val("");
                                $('#confirmpassword').val("");
                                $('#role').val("").change();
                                $('#factory').val("").change();

                                setTimeout(function() {
                                    $('#username').focus();
                                }, 500)
                            });
                        }
                    });
                } else {
                    swal.fire({
                        icon: 'warning',
                        title: 'Warning!',
                        text: 'Please fill in all fields.',
                        showCancelButton: false,
                        showConfirmButton: true
                    }).then(function() {
                        $('#btn_save').prop('disabled', false);
                        $('#btn_save').html('Save');
                    });
                }
            } else {
                $('#warningPassword').show();
                $('#warningConfirm').show();
                $('#btn_save').prop('disabled', false);
                $('#btn_save').html('Save');
            }
        });
        let id_user;
        $('#masterUserTable tbody').on('click', '#btn_edit', function() {
            id_user = masterUserTable.row($(this).parents('tr')).data().id_user;

            let usernameModal = masterUserTable.row($(this).parents('tr')).data().user_name;
            let roleModal = masterUserTable.row($(this).parents('tr')).data().role;
            let factoryModal = masterUserTable.row($(this).parents('tr')).data().idFactory;

            $("#usernameModal").val(usernameModal);
            $('#roleModal').val(roleModal).change();
            $('#factoryModal').val(factoryModal).change();
            // $("#roleModal").val(roleModal);

            $("#editUserModal").modal("show");

        });
        $('#passwordModal').on('click', function() {
            $('#confirmNewPasswordModal').show();
        })
        $('#usernameModal').on('click', function() {
            if ($('#passwordModal').val() == '') {
                $('#confirmNewPasswordModal').hide();
            }
        });
        $('#roleModal').on('change', function() {
            if ($('#passwordModal').val() == '') {
                $('#confirmNewPasswordModal').hide();
            }
        });
        $('#factoryModal').on('change', function() {
            if ($('#passwordModal').val() == '') {
                $('#confirmNewPasswordModal').hide();
            }
        });
        $('#closeModal').on('click', function() {
            $('#passwordModal').val("");
            $('#confirmNewPasswordModal').val("");
            if ($('#passwordModal').val() == '') {
                $('#confirmNewPassword').hide();
            }
        })
        $('#masterUserTable tbody').on('click', '#btn_delete', function() {
            let id_user = masterUserTable.row($(this).parents('tr')).data().id_user;

            swal.fire({
                icon: 'warning',
                title: 'Warning!',
                html: "Are you sure you want to delete this User ?",
                showCancelButton: true,
                // confirmButtonColor: '#3085d6',
                // cancelButtonColor: '#d33',
                confirmButtonText: 'Yes',
                cancelButtonText: 'No',
            }).then(function(result) {
                if (result.value) {

                    $.ajax({
                        type: "POST",
                        url: "<?= site_url('admin/deleteUser') ?>",
                        dataType: "JSON",
                        data: {
                            'id_user': id_user
                        },
                        beforeSend: function() {
                            swal.fire({
                                html: '<h5>Loading...</h5>',
                                showConfirmButton: false,
                                didOpen: function() {
                                    $('.swal2-html-container').prepend(sweet_loader);
                                }
                            });
                        },
                        success: function(data) {
                            masterUserTable.ajax.reload(null, false);
                            swal.fire({
                                icon: 'success',
                                title: 'Success!',
                                text: 'Data deleted successfully.',
                                timer: 1000,
                                showCancelButton: false,
                                showConfirmButton: false
                            });
                        }
                    });


                }


            });
        });
        $('#btn_save_edit_user').on('click', function() {
            let usernameModal = $('#usernameModal').val();
            let passwordModal = $('#passwordModal').val();
            let confirmpasswordModal = $('#confirmpasswordModal').val();
            let roleModal = $('#roleModal').val();
            let factoryModal = $('#factoryModal').val();

            $('#btn_save_edit_user').prop('disabled', true);
            $('#btn_save_edit_user').html('Saving..');

            if (passwordModal === confirmpasswordModal) {
                if (usernameModal !== '' && roleModal !== '') {

                    $.ajax({
                        type: "POST",
                        url: "<?= site_url('admin/updateUser') ?>",
                        dataType: "JSON",
                        data: {
                            'id_user': id_user,
                            'usernameModal': usernameModal,
                            'passwordModal': passwordModal,
                            'roleModal': roleModal,
                            'factoryModal': factoryModal
                        },
                        success: function(data) {
                            masterUserTable.ajax.reload(null, false);
                            $('#confirmNewPasswordModal').hide();
                            $('#warningNewPasswordModal').hide();
                            $('#warningConfirmNewPasswordModal').hide();
                            swal.fire({
                                icon: 'success',
                                title: 'Success!',
                                text: 'Data saved successfully.',
                                timer: 1000,
                                showCancelButton: false,
                                showConfirmButton: false
                            }).then(function() {
                                $('#btn_save_edit_user').prop('disabled', false);
                                $('#btn_save_edit_user').html('Save');
                                $("#editUserModal").modal("hide");
                            });
                        }
                    });
                } else {
                    swal.fire({
                        icon: 'warning',
                        title: 'Warning!',
                        text: 'Please fill in all fields.',
                        showCancelButton: false,
                        showConfirmButton: true
                    }).then(function() {
                        $('#btn_save_edit_user').prop('disabled', false);
                        $('#btn_save_edit_user').html('Save');
                    });
                }
            } else {
                $('#warningNewPasswordModal').show();
                $('#warningConfirmNewPasswordModal').show();
                $('#btn_save_edit_user').prop('disabled', false);
                $('#btn_save_edit_user').html('Save');
            }
        });
    })
</script>